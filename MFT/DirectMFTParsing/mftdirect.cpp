...
// InterlockedAdd64 compatibility for older compilers
#ifndef InterlockedAdd64
#define InterlockedAdd64(pDest, value) InterlockedExchangeAdd64(pDest, value)
#endif
...
static const char* TOOL_VERSION = "2.2.0";  // 2.2.0: Added IsSystemManaged classification, fixed IsTempCache scope
...
// Last access time configuration
...
// Temp/cache path detection patterns
static BOOL IsTempCachePath(const WCHAR* path) {
    // Case-insensitive substring checks for common temp/cache patterns
    ...
    WCHAR lowerPath[MAX_PATH_LEN];
    size_t pathLen = wcslen(path);
    if (pathLen >= MAX_PATH_LEN) pathLen = MAX_PATH_LEN - 1;
    for (size_t i = 0; i < pathLen; i++) {
        lowerPath[i] = towlower(path[i]);
    }
    lowerPath[pathLen] = L'\0';

    for (int i = 0; patterns[i] != NULL; i++) {
        WCHAR lowerPattern[256];
        size_t patLen = wcslen(patterns[i]);
        for (size_t j = 0; j < patLen; j++) {
            lowerPattern[j] = towlower(patterns[i][j]);
        }
        lowerPattern[patLen] = L'\0';

        if (wcsstr(lowerPath, lowerPattern) != NULL) {
            return TRUE;
        }
    }
...
// System-managed path detection patterns
// These are OS-managed directories that users have no practical control over.
// Checked before IsTempCachePath to ensure mutual exclusivity.
static BOOL IsSystemManagedPath(const WCHAR* path, int depth) {
    // Case-insensitive substring checks for Windows system-managed directories
    // Patterns include both with and without trailing backslash to match
    // the directory itself and its children
    const WCHAR* patterns[] = {
        L"\\$Recycle.Bin\\",
        L"\\$Recycle.Bin",
        L"\\RECYCLER\\",
    ...
    // Special case: "Recovery" only at depth 1 (top-level) to avoid false positives
    if (depth == 1) {
        const WCHAR* lastSlash = wcsrchr(path, L'\\');
        if (lastSlash) {
            const WCHAR* dirName = lastSlash + 1;
            if (_wcsicmp(dirName, L"Recovery") == 0) {
                return TRUE;
            }
        }
    }
...
// Only extracts valid extension characters (alphanumeric, underscore, hyphen)
// Stops at comma, space, or other special characters to avoid CSV issues
static void ExtractExtension(const WCHAR* fileName, char* extBuf, size_t extBufSize) {
    extBuf[0] = '\0';
    if (!fileName) return;
    ...
    // Convert to lowercase UTF-8, stopping at invalid extension characters
    // Valid extension chars: a-z, A-Z, 0-9, underscore, hyphen
    WCHAR lowerExt[32];
    size_t extLen = 0;
    for (size_t i = 0; dot[i] && extLen < 31; i++) {
        WCHAR c = dot[i];
        // Stop at any character that's not valid in an extension
        if (c == L',' || c == L' ' || c == L';' || c == L'=' ||
            c == L'"' || c == L'\'' || c == L'(' || c == L')' ||
            c == L'[' || c == L']' || c == L'{' || c == L'}' ||
            c == L'\r' || c == L'\n' || c == L'\t') {
        ...
        lowerExt[extLen++] = towlower(c);
    }

    // If we got nothing or only dots, return empty
    if (extLen == 0) return;

    lowerExt[extLen] = L'\0';

    WideCharToMultiByte(CP_UTF8, 0, lowerExt, -1, extBuf, (int)extBufSize, NULL, NULL);
}
...
static int CsvQuoteUtf8(char* dest, size_t destSize, const char* src) {
    ...
    for (int i = 0; src[i] && pos < (int)destSize - 4; i++) {
        ...
        if (c == '"') {
            ...
            // Replace newlines with space to avoid breaking CSV rows
            dest[pos++] = ' ';
        } else if (c < 0x20 && c != '\t') {
...
struct DirEntry {
    ...
    UCHAR nameSpace;
    // V2 normalized output fields
    int directoryId;         // Assigned ID (1, 2, 3...)
    int parentDirId;         // Parent's directoryId (0 = root)
    int depth;               // Folder depth (0 = volume root)
    BOOL isTempCache;        // Pre-computed temp/cache flag
    BOOL isSystemManaged;    // Pre-computed system-managed flag
    LONGLONG fileCount;      // Direct files in this directory
    LONGLONG totalFileSize;  // Sum of direct file sizes
};
...
static void DirMap_Insert(DirHashMap* map, ULONGLONG recordNum, ULONGLONG parentRecord,
    ...
    map->entries[idx].directoryId = 0;      // Assigned during path resolution
    map->entries[idx].parentDirId = 0;
    map->entries[idx].depth = 0;
    map->entries[idx].isTempCache = FALSE;
    map->entries[idx].isSystemManaged = FALSE;
    map->entries[idx].fileCount = 0;
    map->entries[idx].totalFileSize = 0;
    map->count++;
}
...
// Assign sequential directory IDs and compute v2 fields after all paths are resolved
// Returns the next available DirectoryId (for chaining across volumes)
static int AssignDirectoryIds(DirHashMap* map, int volumeId, int startId) {
    int nextId = startId;

    // First pass: Assign IDs and compute basic fields
    for (size_t i = 0; i < map->capacity; i++) {
        DirEntry* entry = &map->entries[i];
        if (entry->recordNum == 0 || !entry->resolvedPath) continue;

        entry->directoryId = nextId++;

        // Compute depth (count backslashes after volume prefix like "\\?\C:\")
        int depth = 0;
        const WCHAR* p = entry->resolvedPath;
        // Skip "\\?\X:\" prefix (7 chars for drive letter, more for mount points)
        while (*p && *p != L':') p++;
        if (*p == L':') p++;
        if (*p == L'\\') p++;
        // Count remaining path segments
        while (*p) {
            if (*p == L'\\') depth++;
            p++;
        }
        ...
        // Classify: system-managed takes precedence (mutual exclusivity)
        entry->isSystemManaged = IsSystemManagedPath(entry->resolvedPath, entry->depth);
        entry->isTempCache = entry->isSystemManaged ? FALSE : IsTempCachePath(entry->resolvedPath);
    }

    // Second pass: Assign parent directory IDs
    for (size_t i = 0; i < map->capacity; i++) {
        DirEntry* entry = &map->entries[i];
        if (entry->recordNum == 0 || !entry->resolvedPath) continue;

        // Root directory has parentDirId = 0
        if (entry->recordNum == entry->parentRecord) {
            entry->parentDirId = 0;
        } else {
            DirEntry* parent = DirMap_Find(map, entry->parentRecord);
            if (parent && parent->directoryId > 0) {
                entry->parentDirId = parent->directoryId;
            } else {
                entry->parentDirId = 0;  // Orphan or root
            }
        }
    }

    return nextId;  // Return next available DirectoryId for chaining
}
...
struct Pass2Context {
    DirHashMap* dirMap;
    BufferedWriter* filesWriter;  // Renamed from csv for clarity
...
static BOOL Pass2Callback(ULONGLONG recordNum, BYTE* record,
    ...
    // V2: Skip directories in Pass 2 - they're written separately
    if (isDir) {
        ...
        if (attrType == ATTR_FILE_NAME && record[offset + 8] == 0) {
            ...
            if (valLen >= 0x42 && offset + valOff + 0x42 <= recordSize) {
                ...
                if (ns != NS_DOS && fnNameLen > 0 && fnNameLen < 255) {
                    ...
                    int directoryId = 0;
                    if (parentDir && parentDir->directoryId > 0) {
                        directoryId = parentDir->directoryId;
                        // Update directory stats (thread-safe increment)
                        InterlockedIncrement64(&parentDir->fileCount);
                        InterlockedAdd64(&parentDir->totalFileSize, (LONGLONG)dataSize);
                    }

                    // Apply minSize filter
                    if (p->minSizeBytes > 0 && dataSize < p->minSizeBytes) {
                        offset += attrLength;
                        continue;
                    }

                    // Extract extension
                    ExtractExtension(fileName, p->extensionBuf, 64);
                    ...
                    // Build V2 CSV row: DirectoryId,FileName,Extension,FileSize,CreatedTime,ModifiedTime,[AccessedTime,]Attributes
                    int pos = 0;
                    char* row = p->rowBuffer;

                    // DirectoryId
                    pos += snprintf(row + pos, ROW_BUF_SIZE - pos, "%d", directoryId);
                    row[pos++] = ',';
                    ...
                    // Extension
                    int extLen = (int)strlen(p->extensionBuf);
                    if (extLen > 0) {
                        memcpy(row + pos, p->extensionBuf, extLen);
                        pos += extLen;
                    }
                    row[pos++] = ',';

                    // FileSize (use %I64u for ULONGLONG on Windows)
                    pos += snprintf(row + pos, ROW_BUF_SIZE - pos, "%I64u", dataSize);
                    row[pos++] = ',';
                    ...
                    // AccessedTime (only if collecting)
                    if (p->collectAccessTime) {
                    ...
                    BufferedWriter_Write(p->filesWriter, row, (size_t)pos);
...
static int ScanVolumeDirect(const VolumeInfo* vol, BufferedWriter* filesWriter,
                            BufferedWriter* errLog, ULONGLONG minSizeBytes,
                            int volumeId, BOOL collectAccessTime,
                            int startDirectoryId, int* outNextDirectoryId,
                            DirHashMap* outDirMap, VolumeScanResult* outResult) {
    ...
    // V2: Assign sequential directory IDs and compute v2 fields
    wprintf(L"  Assigning directory IDs (starting at %d)...\n", startDirectoryId);
    int nextDirId = AssignDirectoryIds(&dirMap, volumeId, startDirectoryId);
    int dirCount = nextDirId - startDirectoryId;
    wprintf(L"  Assigned %d directory IDs (%d to %d)\n", dirCount, startDirectoryId, nextDirId - 1);
    if (outNextDirectoryId) *outNextDirectoryId = nextDirId;

    // ---- Pass 2: Emit Files CSV ----
    wprintf(L"\n  Pass 2: Writing files CSV...\n");
    ...
    p2.volumeId = volumeId;
    p2.collectAccessTime = collectAccessTime;
    ...
    if (p2.extensionBuf) HeapFree(GetProcessHeap(), 0, p2.extensionBuf);
    ...
    // Transfer directory map to caller for directories.csv output
    if (outDirMap) {
        *outDirMap = dirMap;  // Transfer ownership
    } else {
...
// ============================================================================
// Write Directories CSV (V2 format)
// ============================================================================

static void WriteDirectoriesCsv(const WCHAR* dirCsvPath, DirHashMap* dirMaps,
                                 int volumeCount, int* volumeIds) {
    BufferedWriter dirWriter;
    if (!BufferedWriter_Init(&dirWriter, dirCsvPath, CSV_WRITE_BUF_SIZE, TRUE)) {
        wprintf(L"Error: Cannot create directories CSV: %s\n", dirCsvPath);
        return;
    }

    // Write header: DirectoryId,ScanVolumeId,ParentId,Depth,DirectoryName,FullPath,IsTempCache,IsSystemManaged,FileCount,TotalFileSize
    const char* header = "DirectoryId,ScanVolumeId,ParentId,Depth,DirectoryName,FullPath,IsTempCache,IsSystemManaged,FileCount,TotalFileSize\r\n";
    DWORD written;
    EnterCriticalSection(&dirWriter.lock);
    WriteFile(dirWriter.hFile, header, (DWORD)strlen(header), &written, NULL);
    LeaveCriticalSection(&dirWriter.lock);

    char rowBuffer[MAX_PATH_LEN * 4];
    char utf8Name[1024];
    char quotedName[2048];
    char utf8Path[MAX_PATH_LEN * 3];
    char quotedPath[MAX_PATH_LEN * 6];

    LONGLONG totalDirs = 0;

    for (int v = 0; v < volumeCount; v++) {
        DirHashMap* map = &dirMaps[v];
        int volumeId = volumeIds[v];

        for (size_t i = 0; i < map->capacity; i++) {
            DirEntry* entry = &map->entries[i];
            if (entry->recordNum == 0 || !entry->resolvedPath || entry->directoryId == 0) continue;

            // Convert name and path to UTF-8
            WideCharToMultiByte(CP_UTF8, 0, entry->name, -1, utf8Name, sizeof(utf8Name), NULL, NULL);
            WideCharToMultiByte(CP_UTF8, 0, entry->resolvedPath, -1, utf8Path, sizeof(utf8Path), NULL, NULL);

            // Strip "\\?\" prefix from path for cleaner output
            char* displayPath = utf8Path;
            if (strncmp(displayPath, "\\\\?\\", 4) == 0) {
                displayPath += 4;
            }

            // CSV-quote name and path
            int qnLen = CsvQuoteUtf8(quotedName, sizeof(quotedName), utf8Name);
            int qpLen = CsvQuoteUtf8(quotedPath, sizeof(quotedPath), displayPath);

            // Build CSV row
            int pos = 0;

            // DirectoryId
            pos += snprintf(rowBuffer + pos, sizeof(rowBuffer) - pos, "%d,", entry->directoryId);

            // ScanVolumeId
            pos += snprintf(rowBuffer + pos, sizeof(rowBuffer) - pos, "%d,", volumeId);

            // ParentId
            pos += snprintf(rowBuffer + pos, sizeof(rowBuffer) - pos, "%d,", entry->parentDirId);

            // Depth
            pos += snprintf(rowBuffer + pos, sizeof(rowBuffer) - pos, "%d,", entry->depth);

            // DirectoryName (quoted)
            memcpy(rowBuffer + pos, quotedName, qnLen); pos += qnLen;
            rowBuffer[pos++] = ',';

            // FullPath (quoted)
            memcpy(rowBuffer + pos, quotedPath, qpLen); pos += qpLen;
            rowBuffer[pos++] = ',';

            // IsTempCache
            rowBuffer[pos++] = entry->isTempCache ? '1' : '0';
            rowBuffer[pos++] = ',';

            // IsSystemManaged
            rowBuffer[pos++] = entry->isSystemManaged ? '1' : '0';
            rowBuffer[pos++] = ',';

            // FileCount (use %I64d for LONGLONG on Windows)
            pos += snprintf(rowBuffer + pos, sizeof(rowBuffer) - pos, "%I64d,", entry->fileCount);

            // TotalFileSize
            pos += snprintf(rowBuffer + pos, sizeof(rowBuffer) - pos, "%I64d", entry->totalFileSize);

            // CRLF
            rowBuffer[pos++] = '\r';
            rowBuffer[pos++] = '\n';

            BufferedWriter_Write(&dirWriter, rowBuffer, (size_t)pos);
            totalDirs++;
        }
    }

    BufferedWriter_Destroy(&dirWriter);
    wprintf(L"Directories:  %s (%lld entries)\n", dirCsvPath, totalDirs);
}
...
struct ScanConfig {
    ...
    BOOL allVolumes;
    BOOL showHelp;
    // V2 normalized output settings
    LastAccessConfig lastAccessConfig;
    BOOL collectAccessTime;  // TRUE if access times should be collected
};

static void PrintUsage(void) {
    wprintf(L"mftdirect - Direct MFT Parser for Space Utilization Reporting (v2)\n\n");
    wprintf(L"Usage:\n");
    ...
    wprintf(L"Options:\n");
    wprintf(L"  --output <base>       Output file base name\n");
    wprintf(L"                        (default: mftdirect_YYYYMMDD_HHMMSS)\n");
    ...
    wprintf(L"Output (Schema v2 - normalized format):\n");
    wprintf(L"  <base>.json              Manifest with scan metadata\n");
    wprintf(L"  <base>_directories.csv   Directory tree with IDs\n");
    wprintf(L"  <base>_files.csv         Files referencing directory IDs\n");
    wprintf(L"  <base>_errors.log        Scan errors\n\n");
    wprintf(L"Access times are collected only if NtfsDisableLastAccessUpdate\n");
    wprintf(L"is disabled on the target system (rare).\n\n");
    wprintf(L"Examples:\n");
    wprintf(L"  mftdirect.exe C:\n");
    wprintf(L"  mftdirect.exe C: D: --output scan_results\n");
...
static void BuildDefaultOutputPath(WCHAR* path, size_t pathSize) {
    SYSTEMTIME st;
    GetLocalTime(&st);
    // V2: Output base name without .csv - we'll append _directories.csv, _files.csv
    swprintf(path, pathSize, L"mftdirect_%04d%02d%02d_%02d%02d%02d",
             st.wYear, st.wMonth, st.wDay,
             st.wHour, st.wMinute, st.wSecond);
}
...
static void WriteJsonManifest(const WCHAR* basePath, const WCHAR* dirCsvPath,
                              const WCHAR* filesCsvPath, const WCHAR* errorLogPath,
                              const ScanConfig* config,
                              const VolumeScanResult* results,
                              double totalDurationSec,
                              LONGLONG totalEntries, LONGLONG totalDirs, LONGLONG totalFiles) {

    // Build JSON path: replace _files.csv with .json
    WCHAR jsonPath[MAX_PATH_LEN];
    wcscpy(jsonPath, basePath);
    size_t len = wcslen(jsonPath);
    if (len >= 10 && _wcsicmp(jsonPath + len - 10, L"_files.csv") == 0) {
        wcscpy(jsonPath + len - 10, L".json");
    } else if (len >= 4 && _wcsicmp(jsonPath + len - 4, L".csv") == 0) {
    ...
    const WCHAR* dirFile = wcsrchr(dirCsvPath, L'\\');
    dirFile = dirFile ? dirFile + 1 : dirCsvPath;
    char dirFileUtf8[MAX_PATH];
    WideCharToMultiByte(CP_UTF8, 0, dirFile, -1, dirFileUtf8, sizeof(dirFileUtf8), NULL, NULL);

    const WCHAR* filesFile = wcsrchr(filesCsvPath, L'\\');
    filesFile = filesFile ? filesFile + 1 : filesCsvPath;
    char filesFileUtf8[MAX_PATH];
    WideCharToMultiByte(CP_UTF8, 0, filesFile, -1, filesFileUtf8, sizeof(filesFileUtf8), NULL, NULL);
    ...
    // Convert last access status to UTF-8
    char lastAccessStatusUtf8[128];
    WideCharToMultiByte(CP_UTF8, 0, config->lastAccessConfig.description, -1,
                        lastAccessStatusUtf8, sizeof(lastAccessStatusUtf8), NULL, NULL);

    // Write JSON
    WriteFile(hFile, "{\n", 2, &written, NULL);

    n = snprintf(buf, sizeof(buf), "  \"schemaVersion\": %d,\n", SCHEMA_VERSION);
    ...
    // Last access time section
    WriteFile(hFile, "  \"lastAccessTime\": {\n", 22, &written, NULL);
    n = snprintf(buf, sizeof(buf), "    \"registryValue\": %lu,\n", config->lastAccessConfig.registryValue);
    WriteFile(hFile, buf, n, &written, NULL);
    WriteFile(hFile, "    \"status\": ", 14, &written, NULL);
    WriteJsonString(hFile, lastAccessStatusUtf8);
    WriteFile(hFile, ",\n", 2, &written, NULL);
    n = snprintf(buf, sizeof(buf), "    \"enabled\": %s,\n", config->lastAccessConfig.isEnabled ? "true" : "false");
    WriteFile(hFile, buf, n, &written, NULL);
    n = snprintf(buf, sizeof(buf), "    \"collected\": %s\n", config->collectAccessTime ? "true" : "false");
    WriteFile(hFile, buf, n, &written, NULL);
    WriteFile(hFile, "  },\n", 5, &written, NULL);

    // Output files section
    WriteFile(hFile, "  \"outputFiles\": {\n", 19, &written, NULL);
    WriteFile(hFile, "    \"directories\": ", 19, &written, NULL);
    WriteJsonString(hFile, dirFileUtf8);
    WriteFile(hFile, ",\n", 2, &written, NULL);
    WriteFile(hFile, "    \"files\": ", 13, &written, NULL);
    WriteJsonString(hFile, filesFileUtf8);
    WriteFile(hFile, ",\n", 2, &written, NULL);
    WriteFile(hFile, "    \"errors\": ", 14, &written, NULL);
    WriteJsonString(hFile, errFileUtf8);
    WriteFile(hFile, "\n", 1, &written, NULL);
    WriteFile(hFile, "  },\n", 5, &written, NULL);

    // Totals section
    WriteFile(hFile, "  \"totals\": {\n", 14, &written, NULL);
    n = snprintf(buf, sizeof(buf), "    \"entries\": %I64d,\n", totalEntries);
    WriteFile(hFile, buf, n, &written, NULL);
    n = snprintf(buf, sizeof(buf), "    \"directories\": %I64d,\n", totalDirs);
    WriteFile(hFile, buf, n, &written, NULL);
    n = snprintf(buf, sizeof(buf), "    \"files\": %I64d\n", totalFiles);
    WriteFile(hFile, buf, n, &written, NULL);
    WriteFile(hFile, "  },\n", 5, &written, NULL);
...
int wmain(int argc, wchar_t* argv[]) {
    ...
    // Check last access time status
    config.lastAccessConfig = GetLastAccessTimeStatus();
    config.collectAccessTime = config.lastAccessConfig.isEnabled;

    if (config.outputPath[0] == L'\0') {
        BuildDefaultOutputPath(config.outputPath, MAX_PATH_LEN);
    }

    // Build V2 output paths: base_directories.csv, base_files.csv, base_errors.log
    WCHAR basePath[MAX_PATH_LEN];
    wcscpy(basePath, config.outputPath);
    size_t baseLen = wcslen(basePath);
    if (baseLen >= 4 && _wcsicmp(basePath + baseLen - 4, L".csv") == 0) {
        basePath[baseLen - 4] = L'\0';
    }

    WCHAR dirCsvPath[MAX_PATH_LEN];
    WCHAR filesCsvPath[MAX_PATH_LEN];
    WCHAR errorLogPath[MAX_PATH_LEN];
    swprintf(dirCsvPath, MAX_PATH_LEN, L"%s_directories.csv", basePath);
    swprintf(filesCsvPath, MAX_PATH_LEN, L"%s_files.csv", basePath);
    swprintf(errorLogPath, MAX_PATH_LEN, L"%s_errors.log", basePath);

    // Print banner
    wprintf(L"============================================================\n");
    wprintf(L"MFTDIRECT v%S - Direct MFT Parser (Schema v%d)\n", TOOL_VERSION, SCHEMA_VERSION);
    wprintf(L"============================================================\n");
    wprintf(L"Volumes:    ");
    for (int i = 0; i < config.volumeCount; i++) {
    ...
    wprintf(L"Access time: %s (collect: %s)\n",
            config.lastAccessConfig.description,
            config.collectAccessTime ? L"yes" : L"no");
    wprintf(L"Output:      %s_*.csv\n", basePath);
    wprintf(L"============================================================\n");

    // Open files CSV writer
    BufferedWriter filesWriter;
    if (!BufferedWriter_Init(&filesWriter, filesCsvPath, CSV_WRITE_BUF_SIZE, TRUE)) {
        wprintf(L"Error: Cannot create files CSV: %s\n", filesCsvPath);
        return 1;
    }

    // Write files CSV header (V2 format)
    const char* filesHeader = config.collectAccessTime
    ...
    DWORD headerWritten;
    EnterCriticalSection(&filesWriter.lock);
    WriteFile(filesWriter.hFile, filesHeader, (DWORD)strlen(filesHeader), &headerWritten, NULL);
    LeaveCriticalSection(&filesWriter.lock);
    ...
    LONGLONG totalDirsAllVolumes = 0;
    LONGLONG totalFilesAllVolumes = 0;

    VolumeScanResult volumeResults[MAX_VOLUMES];
    memset(volumeResults, 0, sizeof(volumeResults));

    // Directory maps for each volume (to write directories.csv after all scans)
    DirHashMap dirMaps[MAX_VOLUMES];
    memset(dirMaps, 0, sizeof(dirMaps));
    int volumeIds[MAX_VOLUMES];
    int nextDirectoryId = 1;  // Global DirectoryId counter across all volumes

    for (int v = 0; v < config.volumeCount; v++) {
        volumeIds[v] = v + 1;  // Volume IDs are 1-based

        ScanVolumeDirect(&config.volumes[v], &filesWriter, &errLog,
                         config.minSizeBytes, volumeIds[v], config.collectAccessTime,
                         nextDirectoryId, &nextDirectoryId,
                         &dirMaps[v], &volumeResults[v]);

        totalEntriesAllVolumes += volumeResults[v].entryCount;
        totalDirsAllVolumes += volumeResults[v].directoryCount;
        totalFilesAllVolumes += volumeResults[v].fileCount;
    }

    BufferedWriter_Destroy(&filesWriter);
    BufferedWriter_Destroy(&errLog);

    // Write directories CSV
    wprintf(L"\nWriting directories CSV...\n");
    WriteDirectoriesCsv(dirCsvPath, dirMaps, config.volumeCount, volumeIds);

    // Clean up directory maps
    for (int v = 0; v < config.volumeCount; v++) {
        DirMap_Destroy(&dirMaps[v]);
    }
    ...
    // Write JSON manifest
    WriteJsonManifest(filesCsvPath, dirCsvPath, filesCsvPath, errorLogPath, &config,
                      volumeResults, overallElapsed, totalEntriesAllVolumes,
                      totalDirsAllVolumes, totalFilesAllVolumes);
