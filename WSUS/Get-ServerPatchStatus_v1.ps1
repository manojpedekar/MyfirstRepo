...
.DESCRIPTION
    ...
    Collected signals:
        ...
        * Windows Update Agent history: last quality update (excludes Defender
          definitions) and last update of any kind, with install dates
...
.EXAMPLE
    .\Get-ServerPatchStatus_v1.ps1 -ComputerName SRV01,SRV02 -Verbose

.EXAMPLE
    .\Get-ServerPatchStatus_v1.ps1 -Path C:\temp\servers.txt -Credential (Get-Credential)

.EXAMPLE
    $r = .\Get-ServerPatchStatus_v1.ps1 -Path .\hosts.csv -ThrottleLimit 50
...
begin {
    ...
    $script:LogFile = Join-Path $OutputFolder "Get-ServerPatchStatus_$runTimestamp.log"
    $script:CsvFile = Join-Path $OutputFolder "Get-ServerPatchStatus_$runTimestamp.csv"
    ...
    $collectionScript = {
        ...
        try {
            ...
            try {
                $defRegex = 'Defender|Antivirus|Definition|Security Intelligence|Antimalware'
                ...
                if ($total -gt 0) {
                    ...
                    if ($installed.Count -gt 0) {
                        ...
                        $quality = $installed | Where-Object { $_.Title -notmatch $defRegex } | Select-Object -First 1
                        if ($quality) {
