...
$CollectionScript = {
    param($StartTime, $EndTime, $IsSqlHost, $IncidentTime)
    ...
    try {
        ...
        $systemState = [PSCustomObject]@{
            ...
            # TRUE only if the box booted INSIDE the incident window (i.e. it actually
            # restarted around the incident). A boot after $EndTime is unrelated.
            RebootedInWindow= ($boot -ge $StartTime -and $boot -le $EndTime)
        ...
        $systemState = [PSCustomObject]@{
            ...
            LastBootUpTime = $null; UptimeHours = $null; RebootedInWindow = $null
        ...
        try {
            ...
            foreach ($base in ($logPaths | Select-Object -Unique)) {
                ...
                foreach ($f in $files) {
                    ...
                    # NOTE: [ref] must target a pre-typed [datetime] variable; binding
                    # [ref]$null breaks TryParse overload resolution in a remote runspace
                    # (this was the v1 bug: "Cannot find an overload for TryParse").
                    Get-Content -Path $f.FullName -ErrorAction SilentlyContinue | ForEach-Object {
                        $line = $_
                        if ($line -match '^(\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2})') {
                            $tsText = $matches[1]
                            [datetime]$ts = [datetime]::MinValue
                            if ([datetime]::TryParse($tsText, [ref]$ts)) {
                                if ($ts -ge $StartTime -and $ts -le $EndTime) {
                                    $sqlErrorLog += [PSCustomObject]@{
                                        Server = $server; File = $f.Name
                                        TimeCreated = $ts; Line = $line.Trim()
                                    }
                                }
                            }
                        }
...
if ($Sequential -or $AllServers.Count -eq 1) {
    foreach ($srv in $AllServers) {
        ...
        try {
            ...
            $results += Invoke-Command @icmCommon -ComputerName $srv `
                -ArgumentList $StartTime, $EndTime, $isSql, $IncidentTime
            Write-Log "  ...done: $srv"
        }
        catch {
    ...
    foreach ($srv in $AllServers) {
        ...
        try {
            $results += Invoke-Command @icmCommon -ComputerName $srv `
                -ArgumentList $StartTime, $EndTime, $isSql, $IncidentTime -AsJob -JobName $srv | Out-Null
        }
        catch {
...
foreach ($s in ($results.SystemState | Sort-Object Server)) {
    $flag = if ($s.RebootedInWindow -eq $true) { '  *** REBOOTED DURING INCIDENT WINDOW ***' } else { '' }
...
# Center the excerpt on the incident time - the lines nearest 19:34 are what matter,
# NOT the first N lines of the window (which are just early backup/logon noise).
$sqlLines = $results.SqlErrorLog | Where-Object { $_ -and $_.TimeCreated } | Sort-Object TimeCreated
if ($sqlLines) {
    $window = New-TimeSpan -Minutes 20
    $near = $sqlLines | Where-Object {
        [math]::Abs(($_.TimeCreated - $IncidentTime).TotalMinutes) -le $window.TotalMinutes
    }
    [void]$sb.AppendLine("`n>> SQL ERRORLOG lines within +/-20 min of incident ($($IncidentTime.ToString($TimeStampFormat))):")
    if ($near) {
        $near | ForEach-Object {
        ...
        [void]$sb.AppendLine("   (none in this band - SQL logged nothing near the incident; see full CSV)")
    }

    # Flag any genuinely severe engine errors anywhere in the window (transport,
    # connection, resource, non-yielding scheduler, I/O). 18456 login noise excluded.
    $severe = $sqlLines | Where-Object {
        $_.Line -match '(?i)transport|connection|Severity:\s*(1[6-9]|2[0-5])|17883|17884|non-yielding|latch|I/O requests taking longer|833|9002|deadlock|out of memory|SQLState'
    }
    [void]$sb.AppendLine("`n>> SEVERE SQL ERRORLOG lines in window (login-failure noise excluded):")
    if ($severe) {
        $severe | ForEach-Object {
            [void]$sb.AppendLine(("   {0}  {1}" -f $_.TimeCreated.ToString($TimeStampFormat), $_.Line))
        }
    }
    else {
        [void]$sb.AppendLine("   NONE - SQL engine logged no transport/connection/resource errors. SQL side was healthy.")
    }
}
