...
This script will query for the items which make up the token and then calculate the 
token size based on that dynamic result using the formula in KB327825. 
It will also give you a total of how many SIDs are in the SIDHistory for the user, 
how many of each group scope the user has, and whether the account is trusted for delegation 
or not (if it is the token size may be much larger).

The script has had a major rewrite and now can be run against a single user or a collection of 
users to gauge their estimate token size and provide information about where the "bloat" or size 
is coming from-specific groups, types of groups, group SIDHistory SIDs, user SIDHistory SIDs 
or Windows Kerberos claims (for Windows 8/Server 2012 or later computers).
...
Param ([array]$Principals = ($env:USERNAME),
	$OSEmulation = $false,
	$Details = $false)
...
ForEach ($Principal In $Principals) {
	...
	ForEach ($GroupSid In $Groups) {
		...
		If ($Group.name -ne $null) {
			...
			If (($SIDHistorySids | Measure-Object).Count -gt 0) { $AllGroupSIDHistories += $SIDHistorySids }
	...
	If ($Details -eq $true) {
		...
		If ($GroupSIDHistoryDetails -eq $null) { "[NONE FOUND]" | Out-File $ExportFile -Append } Else { $GroupSIDHistoryDetails | Format-List * | Out-File -FilePath $ExportFile -width 500 -Append }
		"`n" | Out-File $ExportFile -Append
		"User SIDHistory Details" | Out-File $ExportFile -Append
		If ($UserSIDHistoryDetails -eq $null) { "[NONE FOUND]" | Out-File $ExportFile -Append } Else { $UserSIDHistoryDetails | Format-List * | Out-File -FilePath $ExportFile -width 500 -Append }
		"`n" | Out-File $ExportFile -Append
