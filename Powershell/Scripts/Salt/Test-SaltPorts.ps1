...
$SaltExists = Test-Path C:\salt\salt-call.bat
...
if ($SaltExists)
{
	...
	if ($CompName.trim() -eq $Minion_ID.trim()) {
	...
	if (($Version = (salt-call --version).replace("salt-call ", "")) -eq "3003.3") { Write-Host "   Version is correct" -ForegroundColor Green }
...
if ($tcpClient | gm -Name ConnectAsync)
{
	...
	$NetworkPorts = @('System,IP,Port',
		...
		'Zabbix,10.225.142.117,10051') | ConvertFrom-Csv | Select-Object *, Connected
	...
	{
		$tcpClient = New-Object System.Net.Sockets.TcpClient
		$portOpened = $tcpClient.ConnectAsync($NetworkPort.IP, $NetworkPort.Port).Wait($Timeout)
		$NetworkPort.Connected = $portOpened
	}
	
	$NetworkPorts
}else { Write-Host "   This OS does not support the System.Net.Sockets.TcpClient ConnectAsync method" -ForegroundColor Red}
